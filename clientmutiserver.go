...
var hostname [hostnum]string = [hostnum]string{"https://socks-server-758011.asia1.kinto.io", "https://socks-server-1-758011.asia1.kinto.io", "https://socks-server-1-758011.asia1.kinto.io"}
...
func socks5handshark(conn net.Conn, index int) bool {
	...
	if req.atyp != 0x01 {
		...
		hc := &http.Client{}
		hreq, _ := http.NewRequest("POST", hostname[index%hostnum]+"/handshark", body)
...
func post(conn net.Conn, index int) {
	...
	for {
		...
		hreq, _ := http.NewRequest("POST", hostname[index%hostnum]+"/post", body)
...
func get(conn net.Conn, index int) {
	...
	//var httpbody [bufmax]byte //httpbody缓冲区
	hc := &http.Client{}
	hreq, _ := http.NewRequest("GET", hostname[index%hostnum]+"/get", nil)
